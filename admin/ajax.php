...
/*if($_POST['email']){
    ...
    if($check->num_rows > 0){
        ...
        if($row['status'] == 'pending'){
            echo 2; // Account pending verification
        } else {
            echo 1; // Email already exists and verified
        }
    } else {
        // Insert new user with status 'pending'
        $conn->query("INSERT INTO users (email, password, status) VALUES ('$email', '".md5($_POST['password'])."', 'pending')");
        echo 3; // New account created, needs verification
    }
}*/
